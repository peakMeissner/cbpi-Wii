...
class WiiThread(threading.Thread):
    ...
    def stop(self):
        self.running = False
        print("Disconnect the WiiBoard")
        
        try:
            # Disconnect already-connected devices.
            # This is basically Linux black magic just to get the thing to work.
            subprocess.check_output(["bluez-test-input", "disconnect", self.address], stderr=subprocess.STDOUT)
            subprocess.check_output(["bluez-test-input", "disconnect", self.address], stderr=subprocess.STDOUT)
        except:
            pass
...
class WiiSensor(SensorPassive):
    ...
    def read(self):
        if self.unit_of_measure == "KG":
            ...
            self.data_received(round((self.t.value * 2.20462262185) + float(self.offset), 2))
